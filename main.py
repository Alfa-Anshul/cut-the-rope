...
def init_db():
    c = db()
    c.execute('''CREATE TABLE IF NOT EXISTS users(id INTEGER PRIMARY KEY AUTOINCREMENT,username TEXT UNIQUE NOT NULL,email TEXT UNIQUE NOT NULL,password TEXT NOT NULL,token TEXT,created_at TEXT DEFAULT CURRENT_TIMESTAMP)''')
    c.execute('''CREATE TABLE IF NOT EXISTS scores(id INTEGER PRIMARY KEY AUTOINCREMENT,user_id INTEGER NOT NULL,level INTEGER NOT NULL,stars INTEGER NOT NULL,score INTEGER NOT NULL,ts TEXT DEFAULT CURRENT_TIMESTAMP,FOREIGN KEY(user_id) REFERENCES users(id))''')
    c.commit(); c.close()
...
def leaderboard():
    c = db()
    rows = c.execute("SELECT u.username, SUM(s.score) as total, SUM(s.stars) as stars FROM scores s JOIN users u ON s.user_id=u.id GROUP BY u.id ORDER BY total DESC LIMIT 20").fetchall()
    c.close()
